...
describe("20. Publicar pagina creando tag número sin dar click add.", function () {
...
function createPage(path, url) {
  ...
  const tag = faker.datatype.number();
  cy.wait(1000);
  ...
  cy.wait(1000);
  cy.get(".ember-power-select-trigger-multiple-input").first().type(tag);
  cy.wait(1000);
  cy.get(
  ...
  cy.visit(url+"tags");
  cy.wait(1000);
  cy.get(".gh-tag-list-name").contains(tag).should("not.exist");
  cy.wait(1000);
  cy.visit(url+"pages/");
  cy.wait(1000);
