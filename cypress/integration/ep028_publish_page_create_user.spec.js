...
describe("28. Publicar pagina creando usuario número sin dar click add.", function () {
...
function createPage(path, url) {
  ...
  const user = faker.datatype.number();
  cy.wait(1000);
  ...
  cy.wait(1000);
  cy.get(".ember-power-select-trigger-multiple-input").eq(1).type(user);
  cy.wait(1000);
  cy.get(
  ...
  cy.visit(url+"staff");
  cy.wait(1000);
  cy.get(".apps-grid").contains(user).should("not.exist");
  cy.wait(1000);
  cy.visit(url+"pages/");
